...
def update_gail_loop(cell_source):
    if "for run_idx in range(max_runs):" in cell_source:
         # Check if already updated - look for unique string from new block
        if "plot_comparative_dashboard" in cell_source:
             print("GAIL loop already updated.")
             return cell_source
             
        print("Updating GAIL loop logic...")
        ...
        # 3. REPLACE PLOTTING LOGIC WITH REGEX
        # Pattern: try: -> ... -> plot_intermediate_results(...) -> except ... -> print(...)
    ...
    try:
        ...
        # Regex pattern to find the try-except block calling plot_intermediate_results
        # strict matching on "plot_intermediate_results" line to avoid false positives
        regex_pattern = r"(try:\s+plot_intermediate_results\(.*?\)\s+except Exception as e:\s+print\(.*?\))"
        
        # Determine indentation from the match if possible, but here we replace with our indented block.
        # The regex search might fail if indentation matches poorly.
        # Let's try a simpler replace on the function call line, wrapped in try/except logic manually if needed.
        # But we want to replace the whole try-except block.
        
        # Let's try replacing just the function call line and surrounding try/except by context.
        # Use a large window regex.
        pattern = r"(\s+try:\s+plot_intermediate_results\(expert_success_history,\s*all_results,\s*n_episodes=n_train_episodes\)\s+except Exception as e:\s+print\(f\"Plotting failed: {e}\"\))"
        
        matches = re.search(pattern, new_source, re.DOTALL)
        if matches:
            print("Found plotting block match!")
            new_source = new_source.replace(matches.group(1), "\n" + plot_block_new_gail)
        else:
            print("Plotting block NOT found by regex. trying looser match.")
            # Looser match
            pattern_loose = r"(try:\s+plot_intermediate_results.*?except Exception as e:.*?print\(.*?\))"
            new_source = re.sub(pattern_loose, plot_block_new_gail, new_source, flags=re.DOTALL)
