...
class TrackingCallback(BaseCallback):
    """
    Callback for tracking episode rewards and success rates, 
    and stopping training after a fixed number of episodes.
    """
    def __init__(self, n_episodes=500, print_every=50, verbose=0):
        super().__init__(verbose)
        self.n_episodes = n_episodes
        self.print_every = print_every
        self.episode_count = 0
        self.scores = []        # History of all scores
        self.successes = []     # History of all success flags
        self.recent_scores = [] 
    ...
    def _on_step(self) -> bool:
        ...
        for i, done in enumerate(dones):
            if done:
                ...
                # Extract reward and success
                # SB3 Monitor wrapper puts 'episode' key: {'r': reward, 'l': length, 't': time}
                score = 0
                if 'episode' in info:
                    score = info['episode']['r']
                
                # Check for success flag
                # FlattenGoalWrapper or RolloutInfoWrapper might hide it?
                # Usually standard gym/panda_gym puts it in info['is_success']
                # But when wrapped, sometimes it's in info or info['terminal_observation'] etc.
                is_success = 0.0
                ...
                if self.episode_count % self.print_every == 0:
                     ...
                     success_count = int(np.sum(self.recent_successes))
                     success_rate = (success_count / len(self.recent_successes)) * 100
                     
                     print(f"Episode: {self.episode_count} \t "
                           f"Avg Score: {avg_score:.1f} \t "
                           f"Success: {success_count}/{len(self.recent_successes)} "
                           f"({success_rate:.1f}%)")
                     
                     # Reset recent buffers
                     self.recent_scores = []
                     self.recent_successes = []
                
                # Stop if reached n_episodes
